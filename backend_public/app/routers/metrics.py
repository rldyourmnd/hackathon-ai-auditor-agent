...
def list_metric(metric_name: str, db: Session = Depends(get_db), run_id: Optional[int] = None, node_key: Optional[str] = None, limit: int = Query(default=200, le=2000), since: Optional[datetime] = None, until: Optional[datetime] = None):
...
def cumulative(metric_name: str, db: Session = Depends(get_db)):
...
def simple_moving_average(metric_name: str, window: int = Query(default=7, ge=1, le=365), db: Session = Depends(get_db)):
